...
if(! isset($token)) {
		...
		/* The servername and serverport tells PayPal where the buyer
		   ...
		   */
		   ...
		   $i=1;
		   foreach($_REQUEST['L_NAME'] as $pnames)
		   {
		     $products[]='L_NAME'.$i.'='.strip_tags($pnames);
			 $i++;
		   }
		   $product=implode('&',$products);
		  
		   $i=1;
		   foreach($_REQUEST['L_AMT'] as $amt)
		   {
		     $amount[]='L_AMT'.$i.'='.(float)$amt;
			 $i++;
		   }
		   $amounts=implode('&',$amount);
		   $i=1;
		   foreach($_REQUEST['L_QTY'] as $qty)
		   {
		     $quantity[]='L_QTY'.$i.'='.(int)$qty;
			 $i++;
		   }
           $quantitys=implode('&',$quantity);	  

          $productstrings=$product.'&'.$amounts.'&'.$quantitys;		 
          $shippingamt=$_REQUEST['SHIPPINGAMT'];		  
		  $totalamount=$_REQUEST['totalamount'];
		  
		   $itemamt = $totalamount-$shippingamt;
           $amt = $totalamount;
           $maxamt= $amt+25.0;
           $nvpstr="";
	 ...
	 /* The returnURL is the location where buyers return when a
		 ...
		 /* Construct the parameter string that describes the PayPal payment
           ...
           $nvpstr="&ADDRESSOVERRIDE=1$shiptoAddress&$productstrings&MAXAMT=".(string)$maxamt."&AMT=".(string)$amt."&ITEMAMT=".(string)$itemamt."&ReturnUrl=".$returnURL."&CANCELURL=".$cancelURL ."&CURRENCYCODE=".$currencyCodeType."&PAYMENTACTION=".$paymentType."&SHIPPINGAMT=$shippingamt";
		   ...
		   $resArray=paypalc::hash_call("SetExpressCheckout",$nvpstr);
